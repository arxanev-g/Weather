...
const std::string kTestApiKey = "8f883a70-1455-4c03-b026-81f203c4e624";
...
bool ExpectMethod(Poco::Net::HTTPRequest& request, std::string_view method) {
...
bool ExpectKey(Poco::Net::HTTPRequest& request, std::string_view key) {
...
private:
    static bool CheckValue(const std::string& actual, double expected) {
        try {
            return std::abs(std::stod(actual) - expected) < 1e-9;
...
public:
    ...
    void Start() {
        ...
        auto factory = std::make_unique<FakeHandlerFactory>(test_);
        auto* params = new Poco::Net::HTTPServerParams;
        server_ = std::make_unique<Poco::Net::HTTPServer>(factory.release(), *socket_, params);
        server_->start();
    }
